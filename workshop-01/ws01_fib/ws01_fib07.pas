(*****************************************************************************
 *
 *  @mainpage Числа Фибоначчи. Рабочая версия. Процедуры.
 *
 *  @section ОПИСАНИЕ
...
(**
    @var    lim   Количество итераций цикла.
            ...
            ПРОБЛЕМА:
                ...
                происходит целочисленое переполнение.
...
(**
    @fn fib
        Процедура вычисления чисел Фибоначчи.
        @param  limit: byte
            передел вычисления, максимальное число которое нам нужно.
        @param  fibar: fibarray_t
            массив с числами Фибоначчи, в который попадет результат вычисления.
...
procedure fib(limit :byte; var fibar :fibarray_t);
    (**
        @var    counter Счетчик цикла внутри процедуры.
    ...
    **)
    ...
    begin
        (**
            ...
            и если в результате процедуры мы увидим первый элемент 
        ...
        if ((1 > limit) or (limit > 46)) then
            ...
            **)
            ...
            (**
                ...
                и предпредыдущего элемента. Получается, как по определению
                    φ(n) = φ(n-1) + φ(n-2), n ≥ 3.
                Мы смогли отказаться от `prev` и `curr` потому, что их роль выполняют
                соответвующие ячейки массива. А от `temp` мы смогли отказаться,
...
begin
    ...
    **)
    readln(lim);
    (**
        Вызываем процедуру `fib`, и передаем ей в качестве параметров
        число `lim` и массив `fbr`.
        Результат вычисления попадает в массив `fbr`.
    **)
    fib(lim, fbr);
    (**
